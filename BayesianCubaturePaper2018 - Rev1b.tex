...
Under assumptions \eqref{fastcompAssump}, the parameters and credible interval half-widths in Theorem \ref{thm:param} may be expressed in terms of the fast transforms of the function data, the first column of the Gram matrix, and $\vc_\vtheta$ as follows:
\begin{align}
\nonumber
...
A noticeable observation from the plots in all three examples is how much the credible interval half-widths differ from the true error. The credible interval half-widths are not much smaller than $\varepsilon$.  The ratio of the true error to $\varepsilon$ ranges from around one to as small as $10^{-7}$.  In the latter case, the credible interval half-widths are quite conservative.  For option pricing example, this is less serious of an issue than for the multivariate Gaussian and Keister examples. A possible reason is that the latter integrands are significantly smoother than the covariance kernel assumed.  This is a matter for further investigation.
