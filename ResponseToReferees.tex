...
minimized for it. The $C$ without subscript also clashes that of lemma 1. \Response{$\vec{\theta}$ has been added as much as possible, and particularly into the equations where there is optimization.}
...
\item Section 3.3. Elaborate on what is meant with the second sentence.  \Response{This section re-written and eliminaets this sentence.}
...
increase the error on $\mathring{\lambda}_1$, so putting the numerical error elsewhere. \Response{A key quantity can lose accuracy due to catastrophic cancellation.  This has been observed by us in practice.  The proposed method avoids this.  We do not see how we are introducing significantly more numerical error elsewhere.  We are basically pulling a large constant matrix one out of the Gram matrix.  Then the FFT of the first column is not essentially $n$.  We want the difference from $n$.}
...
\item Sec.\ 2.2.1, 2.2.2 I suggest to include some references for the results that are presented in Sections 2.2.1 and 2.2.2.  \Response{Our knowledge of the literature is limited, but much of what we read assumes a zero mean for the Gaussian process.  We would welcome sources to cite that consider our setting.}
...
\item Sec.\ 5.1 Is there some kind of a universal periodization that ‘works’ for every value of $r$?  \Response{One could choose an infinitely smooth periodization, but this would change the integrand to potentially make its covariance $s$ large. This is mentioned at the end of Section 5.1.}
