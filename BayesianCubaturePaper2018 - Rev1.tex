...
\mC^T = \mC,  \quad \va^T \mC \va > 0, \quad \text{where }  \mC = \left(  C_\vtheta(\vx_i,\vx_j)  \right)_{i,j=1}^n,\\
 \text{for all } \va \ne 0, \;
 n\in \mathbb{N}, \qquad
 \vx_1, \ldots, \vx_n \in [0,1]^d, \; \text{s.t} \; \\\vx_i \neq \vx_j \forall i\neq j, \; 1 \leq i, j \leq n.
\end{multline}
The function, $C_{\vtheta}$, and the Gram matrix, $\mC$ depend implicitly on $\vtheta$, but the notation may omit this parameter for simplicity's sake.
...
\end{align}
\end{subequations}
We need the following lemma on conditional normal distribution to derive the distribution of the posterior error of our cubature. 
...
\right] \vy, 
\end{align}
To obtain $\vtheta_\MLE$, substitute $m_\MLE$ and $s_\MLE$ into $l(s,m,\vtheta | \vy)$ and then numerically minimize
\begin{align}
\nonumber
\vtheta_\MLE
...
Under these estimates of the parameters, the cubature \eqref{eqn:BayesCub} and the credible interval \eqref{eqn_prob_confidence_interval} are explicitly written as
...
Rather than use maximum likelihood to determine $m$ and $s$ one can treat them as hyperparameters with a non-informative, conjugate prior, namely $\vrho_{m,s^2}(\xi, \lambda) \propto 1/\lambda$. Let us assume integral $\mu$, a random variable, taking typical values $z$, then the posterior density for the $\mu$ given data $\vy$ using Bayes theorem is
\begin{align*}
...
& \propto \left( 1 +  \frac{(z - \hmu_{\textup{MLE}})^2}{(n-1)\widehat{\sigma}_{\textup{full}}^2} \right)^{-n/2}, \text{  (by Appendix \ref{appendix:full_bayes})}
\end{align*}
where 
\begin{multline*}
...
Because the shape parameter, $\vtheta$, enters the definition of the covariance kernel in a non-trivial way, the only way to treat it as a hyperparameter and assign a tractable prior would be for the prior to be discrete.  We believe in practice that choosing such a prior involves more guesswork, so we choose to use either empirical Bayes estimate of $\vtheta$ in \eqref{eqn:thetaMLE} or cross-validation approach described next.
...
\end{multline}
Plugging the value of $m_\GCV$ into \eqref{eqn:BayesCub} yields
\begin{align}
\label{eqn:muCV}
...
\end{align}
where $\mV^H$ is the Hermitian of $\mV$, $\vC_1,...,\vC_n$ are columns of $\mC$.  $\vV_1,...,\vV_n$ are columns of $\mV$ and $\vv_1,...,\vv_n$ are rows of $\mV$.  The columns of matrix $\mV$ are eigenvectors of $\mC$, and $\mLambda$ is a diagonal matrix of eigenvalues of $\mC$.
...
\begin{appendices}
\section{Details of full Bayes posterior} \label{appendix:full_bayes}
Let us start with given integral $\mu$, a random variable, taking typical values $z$, then the posterior density for the $\mu$ given data $\vy$ using Bayes theorem is
\begin{align*}
\MoveEqLeft[1]{\rho_{\mu}(z | \vf = \vy)} \\
& \propto \int_{0}^\infty \int_{-\infty}^\infty \rho_{\mu}(z | \vf = \vy, m = \xi, s^2 = \lambda) \\
& \hspace{2.5cm} \times  \rho_{\vf}(\vy | \xi, \lambda ) \rho_{m, s^2}(\xi, \lambda) \, \D \xi \D \lambda \\
